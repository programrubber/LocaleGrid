...
- 보고서, 산출물, 기능 점검 문서는 프로젝트 루트가 아닌 `doc/`에 저장한다.
- 파일명은 `YYYY-MM-DD_프로젝트명_한글_문서명_v버전.확장자` 형식을 사용한다. 프로젝트명이나 버전이 없는 문서는 해당 부분을 생략한다.
- 보고서 이미지와 원본 도식은 `doc/report-assets/`에 저장한다.
- 문서를 추가하거나 이름을 변경하면 `doc/문서_목록.md`도 함께 갱신한다.
