...
핵심 목표는 `locales/{locale}/{category}.json` 구조의 JSON 파일을 열었을 때, 기본 JSON 에디터 옆 `Locale Grid` 탭에서 다국어 값을 비교, 수정, 검증, 저장할 수 있게 만드는 것이다.
...
- 상태는 별도 `status` 컬럼으로 두지 않고 key 컬럼 안에 고정폭 배지로 표시한다.
...
- 기본 그리드는 `key`, 언어별 컬럼, `번역 묶음`으로 구성
...
3. `Diagnostic` 구조 확장
4. 저장 전 메모리 검증 강화
5. 그리드 UI를 `key 상태 배지 + 언어별 컬럼 + 번역 묶음 마지막 컬럼 기본 hide + 상세 패널` 구조로 개선
...
- JSON 파일을 열면 `Locale Grid` 탭이 표시된다.
