...
기본 대상 구조는 `locales/{locale}/{category}.json`이며, 같은 category에 속한 여러 locale 파일을 하나의 표로 묶어 보여줍니다.
...
- `locales/{locale}/{category}.json` 구조 자동 인식
- locale 디렉터리 자동 감지
...
- `__comment__`, `__commant__` 같은 구역 표시용 key readonly 표시

## LocaleGrid 확인 방식

LocaleGrid는 별도 메뉴 액션이 아니라 JSON 파일의 커스텀 에디터 탭으로 동작합니다.

1. 프로젝트에서 `locales/{locale}/{category}.json` 형태의 JSON 파일을 엽니다.
...
- `수동 locale 목록`: 쉼표로 구분한 locale 목록. 비어 있으면 자동 감지
- `구역 표시 key`: readonly 구역 표시용 key 목록. 기본값 `__comment__,__commant__`
- `JSON 들여쓰기`: 저장 시 JSON indent. 기본값 `2`

## LocaleGrid 확인 프로세스

1. 현재 연 JSON 파일이 `locales/{locale}/{category}.json` 구조인지 확인합니다.
2. category 이름을 현재 JSON 파일명에서 추출합니다.
