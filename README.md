...
- `사내 AI 번역 제안 (모든 프로젝트 공통)`:
  - AI 설정은 IDE 전체에 저장되어 다른 프로젝트에서도 그대로 사용합니다. 기본·고급 설정은 프로젝트별로 유지됩니다.
  - 기존 버전에서 설정한 값은 글로벌 설정이 없을 때 처음 발견한 사용자 지정 AI 설정을 한 번 가져옵니다. 이후 다른 프로젝트의 기존 설정으로 덮어쓰지 않습니다.
  - 설정을 적용하면 모든 열린 프로젝트의 AI 번역 제안 버튼에 반영됩니다.
  - `기능 활성화`: 하단 상세 패널에서 보라색 AI 번역 제안 버튼 표시 여부. 기존 참조 문장 또는 문구 요청과 편집 가능한 빈 언어 항목이 있을 때 활성화되며, 모든 언어가 입력되면 비활성화됩니다.
  ...
  - 하단의 카테고리·행 수·편집·오류·경고 상태 정보는 항상 유지하며, AI 진행·결과·실패 안내는 `AI 번역 제안` 버튼과 스타일 옵션 영역 오른쪽에 표시합니다. 공간이 부족하면 AI 안내가 말줄임되며 툴팁으로 확인할 수 있습니다.
